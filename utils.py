...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, x, patchwise_msad=False):
        ...
        for end_idx in idx_crops:
            _out = self.backbone(
                torch.cat(x[start_idx:end_idx]), patchwise_msad=patchwise_msad
            )
...
def load_model(
    ...
    if args.pretrained_weights:
        ...
        if "mae" in args.pretrained_weights:
            ...
            mis, unex = model_without_ddp.load_state_dict(
                state_dict_renamed, strict=False
            )
        else:
            state_dict_renamed = {
                k.replace(prefix, ""): v for k, v in checkpoint["student"].items()
            }
            mis, unex = model_without_ddp.load_state_dict(
                state_dict_renamed, strict=False
            )
