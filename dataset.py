...
class CustomImageFolder(ImageFolder):
    def __init__(self, root, transform=None, msad_transform = None, target_transform=None, sensors = None):
        ...
        self.normalizations = {
            "Landsat" : transforms.Normalize([0.27059479, 0.27839213, 0.18060363], [0.1741122 , 0.14797395, 0.125955  ]),
            "Sentinel" : transforms.Normalize([0.14316194, 0.14518686, 0.09228685], [0.13242126, 0.10325284, 0.08168219]),
            "SPOT" : transforms.Normalize([0.31007201, 0.34869021, 0.23991865], [0.16708196, 0.14294321, 0.16296565])}
    ...
    def __getitem__(self, index):
        ...
        if self.msad_transform:
            ...
            batch[f"{sensor}_noaugm"] = self.normalizations[sensor](self.msad_transform(img))
            for oth_sensor in oth_av_sensors:
                oth_im_path = path.replace(sensor, oth_sensor)
                oth_im = Image.open(oth_im_path).convert('RGB')
                batch[f"{oth_sensor}_noaugm"] = self.normalizations[oth_sensor](self.msad_transform(oth_im))
